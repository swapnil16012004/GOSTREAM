...
router.get("/", async(req, res) => {
    ...
    res.render("listings/index.ejs", {marvelListings, historyListings, popularMovieListings, comedyListings, kidListings, currUser});
})
...
router.get("/search", async(req, res) => {
    ...
    res.render("listings/search.ejs", {marvelListings, historyListings, popularMovieListings, comedyListings, kidListings, key});
})

router.get("/new/:show", isLoggedIn, async (req, res) => {
    const category = req.params.show.replace('new', '').charAt(0).toLowerCase()+req.params.show.replace('new', '').slice(1);
    if (!models[category]) {
...
router.post("/:category", upload.single("listing[image]"), async (req, res, next) => {
